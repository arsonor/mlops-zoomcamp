...
```python
data = [
    (None, None, dt(1, 1), dt(1, 10)),         # duration = 9 min → valid
    (1, 1, dt(1, 2), dt(1, 10)),               # duration = 8 min → valid
    (1, None, dt(1, 2, 0), dt(1, 2, 59)),      # duration = 0.98 min → invalid
    (3, 4, dt(1, 2, 0), dt(2, 2, 1)),          # duration = 1441 min → invalid
]
...
Only the first two rows have durations:

Between 1 and 60 minutes
→ ✅ So 2 rows should remain after filtering.
...
Create the S3 bucket (via AWS CLI):
...
After running `integration_test.py`, check the file:
...
We’re now setting up a full integration test:  
✔️ Prepare input data  
✔️ Save it to Localstack S3  
✔️ Run the batch.py prediction pipeline  
✔️ Read the output from S3  
✔️ Validate the results  
